...
\draft{
Other works that analyze \wvtwo models rather focus on the large \transformer part of the network which operates at a higher level of abstraction \cite{livescu2021wav2vec_analysis}.
\TODO{updated version and add more papers (references in Choi et al. or Livescu et al.}
}
...
However, this is always done in the whole \wvtwo framework.
\TODO{Highlight differences to this paper.}
}
...
It is striking that the \wv2 \fe uses two orders of magnitude more parameters than the \gls{SC} architecture.
...
The final projection, which comprises about 394k parameters, additionally improves the performance as shown in \refTab{table:features_w2v_proj}.
...
In particular, it does not seem to be necessary to cover one full \draft{pitch period (max. fundamental period) which is around \ms{16} for low voices}. %\TODO{check detailed explanation.}
...
The plots are depicted in \refFig{fig:first_layer}.
The color range is shared between all subplots.

The well known distribution of the Gammatone filters is given in \refFig{fig:first_layer}\,(a).
